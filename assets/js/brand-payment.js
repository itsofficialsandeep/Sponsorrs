...
localStorage.setItem("brand_paymentsFilterCategory", "all");
localStorage.setItem("brand_paymentCurrentPage", "0");

$(document).ready(function () {
  console.log("=" + localStorage.getItem("brand_paymentCurrentPage"));

  // searching the next page from database
  //var from = localStorage.getItem("brand_paymentCurrentPage") ? localStorage.getItem("brand_paymentCurrentPage") : 0;
  var from = 0;
  var brand_paymentFormData = {
  ...
  $.ajax({
    type: "POST",
    url: "../profile-functions/brand-payment-functions.php",
    data: brand_paymentFormData,
    success: function (response) {
      $("#payment-response-area").html(response);
      localStorage.setItem("brand_paymentCurrentPage", "20");
      console.log($("#payment-filter-option").val());
      console.log(
        "==" + localStorage.getItem("brand_paymentCurrentPage")
      );
    },
...
$("#payment-searchbar").keyup(function (e) {
  ...
  // searching the next page from database
  var from = localStorage.getItem("brand_paymentCurrentPage")
    ? localStorage.getItem("brand_paymentCurrentPage")
    : 0;
...
$("#payment-load-more").click(function (e) {
  var currentPage = localStorage.getItem("brand_paymentCurrentPage");
  var nextPage = parseInt(currentPage) + 2;

  console.log("===" + nextPage);
  var currentCatagory = $("#payment-filter-option").val();

  var reloadbrand_paymentFilter = {
    filter: 1, //$("#payment-filter-option").val()
    actionType: 3,
    from: currentPage,
  };
  ...
  $.ajax({
    type: "POST",
    url: "../profile-functions/brand-payment-functions.php",
    data: reloadbrand_paymentFilter,
    success: function (response) {
      if (response.length < 10) {
        ...
        localStorage.setItem("brand_paymentCurrentPage", nextPage);
      }
    },
    error: function (error) {
      console.error(error);

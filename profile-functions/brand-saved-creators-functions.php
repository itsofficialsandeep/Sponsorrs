...
if (isset($_POST['actionType'])) {
    ...
    if (isset($_POST['actionType']) && $_POST['actionType'] == 1 && isset($_POST['applyId'])) {
        ...
        $creator = $_POST['applyId'];
        $creator = trim($creator, "#");

        $sql = "DELETE FROM saved_channel_profiles WHERE saver_brand_id='$currentUser' AND saved_channel_id='$creator'";
    ...
    if ($actionType == 3) {
        ...
        //     $sql = "SELECT company.id_company,company.logo,company.companyname,company.industry,company.country, COUNT(sponsorships.active) 
        //     AS active_ads from company LEFT JOIN sponsorships on company.id_company = sponsorships.id_company WHERE company.id_company in 
        //     (SELECT DISTINCT saved_brand_profiles.saved_brand_id from saved_brand_profiles where saved_brand_profiles.saver_channel_id=$currentUser) 
        //     and sponsorships.active=1 GROUP BY company.id_company ORDER BY company.companyname ASC LIMIT $from,$total";
        ...
        //     AS active_ads from company LEFT JOIN sponsorships on company.id_company = sponsorships.id_company WHERE company.id_company in 
        //     (SELECT DISTINCT saved_brand_profiles.saved_brand_id from saved_brand_profiles where saved_brand_profiles.saver_channel_id=$currentUser) 
        //     and sponsorships.active=1 and company.companyname LIKE '%$text%' GROUP BY company.id_company ORDER BY company.companyname ASC LIMIT $from,$total";
        // }
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<tr data-pages="' . $from . '" id="' . $owned_by . '">
							...
							<td>
								<button class="btn btn-sm btn-danger-soft btn-round mb-0 deleteSponsorshipButton" onclick="deleteRequest(this.value)" data-sponsorship="' . $owned_by . '"  value="#' . $owned_by . '" ><i class="bi bi-person-dash person-dash fs-5"></i></button>
							</td>

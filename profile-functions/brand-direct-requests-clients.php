		...
		if ($result = $conn->query($sql)) {
			if ($result->num_rows > 0) {
				while ($row = $result->fetch_assoc()) {
					...
					// 	$action = '<button class="btn btn-sm btn-danger-soft btn-round mb-0 deleteSponsorshipButton" onclick="deleteRequest(this.value)" data-sponsorship="' . $apply_id . '"  value="#' . $apply_id . '" ><i class="fas fa-fw fa-times"></i></button>';
					// }

					echo '<tr data-pages="' . $sql . '" id="' . $owned_by . '">
							<!-- Course item -->
							<td>

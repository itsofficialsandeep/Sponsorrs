...
$query = "SELECT `id`, channel_detail.`sr_no`, `snippettitle`, `snippetdescription`, 
				`snippetcustomUrl`, `snippetthumbnailsdefaulturl`, 
				`snippetcountry`, `statisticsviewCount`, `statisticsvideoCount`, `statusprivacyStatus`, `statusmadeForKids`, 
				`statisticssubscriberCount`, `channel_type`, `owned_by` , snippetpublishedAt, 
				contentDetailsrelatedPlaylistsuploads, primary_ac.token_secret, sub_creation.sub_creation_id,				  
				IF(id 
					IN (
						SELECT DISTINCT saved_channel_id 
						FROM saved_channel_profiles 
						WHERE saver_brand_id = '$currentUser'
					)
				,1,0) AS saved, 
				IF(channel_detail.owned_by 
					IN (SELECT applied_creator_id 
						FROM apply_creators 
						WHERE applier_brand_id='$currentUser' 
			...
			FROM `channel_detail` 
				LEFT JOIN primary_ac 
					ON primary_ac.ac_id =channel_detail.owned_by 
				LEFT JOIN sub_creation 
					ON sub_creation.primary_ac = channel_detail.owned_by 
			WHERE id='$channel' 
...
if ($result->num_rows > 0) {
	...
	$owner = $row['owned_by'];
	$id = $row['id'];
	$subscreationId = $row['sub_creation_id'];
	$contentDetailsrelatedPlaylistsuploads = $row['contentDetailsrelatedPlaylistsuploads'];
	$tokenSecret = $row['token_secret'];
	$applied = $row['applied'];
}

// for APPLY button
$token = $id . "|" . $subscreationId . '|' . hash("sha512", $tokenSecret . $_SESSION['currentUser']);
// if the channel has already been applied then show "applied" text
$appliedOrNot = $applied == 1 ? 'Applied' : 'Apply';

$query = "SELECT channel_category, sponsor_type, intro_video, 
				benefit, previous_sponsors, dp_username 
		  FROM users 
		  LEFT JOIN primary_ac 
		  ON users.primary_ac_id = primary_ac.ac_id 
		  WHERE channel_id='$id'";
$result = $conn->query($query);
...
$query = "SELECT IF ('$id' IN (SELECT saved_channel_id FROM saved_channel_profiles WHERE saver_brand_id='$currentUser'), 1, 0 ) as saved";
$result = $conn->query($query);
if ($result->num_rows > 0) {
	$row = $result->fetch_assoc();
	$saved = $row['saved'];
...
$query = "SELECT facebook, twitter, reddit, instagram, youtube_1, youtube_2, youtube_3, tiktok, pinterest, linkedin, snapchat, website FROM social_links WHERE channel_user_id='$owner'";
$result = $conn->query($query);
if ($result->num_rows > 0) {
	$row = $result->fetch_assoc();
...
$publishedAt = date("F jS, Y", strtotime($publishedAt));
					...
					<div class="col-lg-12">
						...
						<div class="d-flex justify-content-between ">
							<p class="d-flex" style="width:75%">
								<?php echo substr($previous_sponsors, 0, 200) . ".."; ?>
							</p>

							<!-- Badge -->
							<button data-action="applyCreators" data-token="<?php echo $token; ?>"
								class="applyCreators me-2 btn btn-primary mb-3 rounded-2 fs-5" id="applyToChannel"><?php echo $appliedOrNot; ?></button>
							<button
								class="d-none me-2 btn btn-lg btn-warning mb-3 font-base py-2 px-4 rounded-2 d-inline-block">Follow</button>
							<button data-action="saveCreators" data-token="<?php echo $id . '|example'; ?>"
								id="saveCreators" class=" btn btn-success mb-3 rounded-2 fs-5"><?php echo $saved; ?></button>
						</div>
						<!-- Content -->
						<ul class="list-inline mb-0">
							<li class="list-inline-item h6 me-3 mb-1 mb-sm-0"><i
									class="fas fa-users text-orange me-2"></i> <span class="small">Subscriber:</span>
								<?php echo customNumberFormat($subscriberCount); ?>
							</li>
							<li class="list-inline-item h6 me-3 mb-1 mb-sm-0"><i
									class="fas fa-birthday-cake text-primary me-2"></i><span class="small">Channel
									Started:</span>
									...
									<div class="tab-pane fade show active" id="course-pills-1" role="tabpanel"
										...
										<div class="card mb-0 mb-md-4">
											<div class="row g-0 align-items-center">
												<div class="col-md-5">
													<!-- Image -->
													<img src="<?php echo str_ireplace('88', '300', $thumbnail) ?>"
												...
												<div class="col-md-7">
													...
													<div class="card-body">
														...
														<ul class="list-inline d-flex align-item-center mt-3"
															style="height:30px">
															<div title="Category of the channel"
																class="d-flex align-items-center me-3 mb-2  align-self-center ">
																<span
																	class="bg-opacity-15 text-primary  align-self-center"><i
																		class="icon-xl bi bi-tag-fill"></i></span>
																<span class="fw-bolder h6 fw-light mb-0 ms-2">
																	<?php echo adcategoryNumToString($channel_category); ?>
																</span>
															</div>
															<div title="Type of sponsorship this channel want"
																class="d-flex align-items-center me-3 mb-2">
																<span class="bg-opacity-15 text-warning"><i
																		class="icon-xl bi bi-badge-ad-fill"></i></span>
																<span class="fw-bolder h6 fw-light mb-0 ms-2">
																	<?php echo adcategoryNumToString($sponsor_type); ?>
																</span>
															</div>
														</ul>
														<hr>
														...
														<?php
														contactBadgeFromArray(emailFromString($description), "email");
														contactBadgeFromArray(phoneFromString($description), "phone");
														contactBadgeFromArray(socialURLFromString2($description), "social");
														?>
													</div>
												</div>
											</div>
										...
										<h5 class="mb-3">Channel Description</h5>
					...
					<div class="col-lg-4 pt-5 pt-lg-0">
						<div class="col-md-6 col-lg-12">
							...
							<div class="card card-body shadow p-4 mb-4">
								<!-- Title -->
								<h4 class="mb-3">More about this channel..</h4>
								<ul class="list-group list-group-borderless">
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0"><i
												class="fas fa-fw fa-users text-primary"></i>Subscribers</span>
										<span>
											<?php echo customNumberFormat($subscriberCount); ?>
										</span>
									</li>
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0"><i
												class="fas fa-fw fa-video text-primary"></i>Videos</span>
										<span>
											<?php echo customNumberFormat($videoCount); ?>
										</span>
									</li>
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0"><i
												class="fas fa-fw fa-eye text-primary"></i>Total Views</span>
										<span>
											<?php echo customNumberFormat($viewCount); ?>
										</span>
									</li>

									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0"><i
												class="fas fa-fw fa-clock text-primary"></i>Channel Started</span>
										<span>
											<?php echo $publishedAt ?>
										</span>
									</li>
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0"><i
												class="fas fa-fw fa-child text-primary"></i>Made for kids</span>
										<span>
											<?php $forKids = $forKids == 0 ? "Public" : "Made For Kids";
											echo $forKids ?>
										</span>
									</li>
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0"><i
												class="fas fa-fw fa-globe text-primary"></i>Country</span>
										<span>
											<?php echo countryCodeToName($conn, $country); ?>
										</span>
									</li>
							...
							<div class="card card-body shadow p-4">
								...
								<ul class="list-inline mb-0">
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=tech reviews">Tech
											Reviewes</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=finance">Finance</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=explainers">Explainers</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=case study">Case
											Study</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=uspsc">UPSC</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=entertainment">Entertainment</a>
									</li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=vlogging">Vlogging</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=news and politics">News
											and Politics</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=food">Food</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=clothe">Clothe</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=cartoon">Cartoon</a> </li>
									<li class="list-inline-item"> <a class="btn btn-outline-light btn-sm"
											href="search.php?searchtype=creators&query=comedy">Comedy</a> </li>
				...
				<div class="row">
					...
					<div class="pb-2 tiny-slider arrow-round arrow-blur arrow-hover">
						<div class="tiny-slider-inner pb-1" data-autoplay="false" data-arrow="true" data-edge="2"
							...
							$suggest->suggestYoutubeCreator($conn, $channel_category, 10, $currentUser); ?>

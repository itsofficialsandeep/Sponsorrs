...
if (isset($_POST['search_result_type'])) {
	...
	if ($searchResultType == 1) {
		...
		if ($_POST['filter'] == 1) {
			$querySponsorshipDetail = "SELECT sponsorship_id, posted_on, sponsorship_title, description,id_company FROM `sponsorships` 
										WHERE active=1 and id_company='$currentUser' ORDER BY posted_on ASC LIMIT $from,$total";
		}
					...
					echo '<tr data-pages="' . $from . '" id="' . $sponsorship_id . '">
							...
							<td>
								<div class="d-flex align-items-center">
									...
									<div class="mb-0 ms-2">
										<!-- Title -->
										<h6><a href="#">' . substr($sponsorship_title, 0, 15) . '</a></h6>
									</div>
								</div>
							</td>
							<td>
